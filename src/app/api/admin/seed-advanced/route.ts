...
export async function POST() {
  ...
  return NextResponse.json({ 
    ...
    message: 'Seed avanzado deshabilitado temporalmente para deploy',export async function POST() {import { PrismaClient } from '@prisma/client';
    ...
    // Función deshabilitada temporalmente para evitar errores de build en Herokuexport async function POST() {import bcrypt from 'bcryptjs';

    return NextResponse.json({ 

      message: 'Seed avanzado deshabilitado temporalmente para deploy',  try {

      status: 'disabled',

      note: 'Esta función será habilitada después del deploy inicial'    // Función deshabilitada temporalmente para evitar errores de build en Herokuconst prisma = new PrismaClient();
    ...
    console.error('Error in seed-advanced:', error)      message: 'Seed avanzado deshabilitado temporalmente para deploy',export async function POST() {

    return NextResponse.json(

      { error: 'Error interno del servidor' },      status: 'disabled',  try {

      { status: 500 }

    )      note: 'Esta función será habilitada después del deploy inicial'    console.log('🚀 Iniciando seed avanzado de base de datos...\n');
  ...
  } catch (error) {    console.log('🧹 Limpiando datos existentes...');

    console.error('Error in seed-advanced:', error)    await prisma.drillingData.deleteMany();

    return NextResponse.json(    await prisma.well.deleteMany();

      { error: 'Error interno del servidor' },    await prisma.field.deleteMany();

      { status: 500 }    await prisma.contract.deleteMany();
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const contract = await prisma.contract.create({
        data: {
          ...
          startDate,
          endDate,
          value: Math.random() * 50000000 + 10000000, // 10-60M USD
          ...
          expectedDays: Math.floor(Math.random() * 100) + 200, // 200-300 días
